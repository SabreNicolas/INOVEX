...
export class PermisFeuComponent implements OnInit {
      ...
      .subscribe((response) => {
        if (response == "Création du permis de feu OK") {
          this.popupService.alertSuccessForm(
            "Le permis de feu/zone de consignation a bien été créé !",
          );
        } else {
          this.popupService.alertErrorForm(
            "Erreur lors de la création du permis de feu/zone de consignation ....",
          );
        }
      });
  }
  ...
  generateDateHeureFin(form: NgForm) {
    this.dateHeureDeb = this.datePipe.transform(
      form.value["dateHeureDeb"],
      "yyyy-MM-dd HH:mm",
    );
