...
export class MoralEntitiesComponent implements OnInit {
      ...
      this.moralEntitiesService
        ...
        .subscribe((response) => {
          ...
          if (this.MR.Code.length > 5) {
            this.produit =
              this.MR.Code.substr(0, 3) + "-" + this.MR.Code.substr(5, 2);
    ...
    this.moralEntitiesService.GetTypeDéchets().subscribe((response) => {
      ...
      this.listTypeDechetsCollecteurs.forEach((typeDechetsCollecteurs) => {
        ...
        if (typeDechetsCollecteurs.Code.length > 5) {
          typeDechets =
            typeDechetsCollecteurs.Code.substring(0, 3) +
            "-" +
            typeDechetsCollecteurs.Code.substring(5) +
            "_" +
            typeDechetsCollecteurs.Name.split(" ")[0];
          collecteur =
  ...
  onSubmit(form: NgForm) {
    ...
    this.moralEntitiesService.unitPrice = +this.unitPrice
      .toString()
      .replace(",", ".");
    ...
    if (this.produit.length > 3) {
      this.code =
        this.produit.substr(0, 3) + this.collecteur + this.produit.substr(4, 2);
    } else {
    ...
    if (this.codeDechet == "" || this.codeDechet == null) {
      ...
      this.moralEntitiesService.codeDechet = this.codeDechet.replace(
        /'/g,
        "''",
      );
    ...
    if (this.nomClient == "" || this.nomClient == null) {
      ...
      this.moralEntitiesService.nomClient = this.nomClient.replace(/'/g, "''");

    if (this.prenomClient == "" || this.prenomClient == null) {
      this.moralEntitiesService.prenomClient = "_";
    } else
      this.moralEntitiesService.prenomClient = this.prenomClient.replace(
        /'/g,
        "''",
      );
    ...
    if (this.mailClient == "" || this.mailClient == null) {
      ...
      this.moralEntitiesService.mailClient = this.mailClient.replace(
        /'/g,
        "''",
      );
    //FIN INFOS SUPP
