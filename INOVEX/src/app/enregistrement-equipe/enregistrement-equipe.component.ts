...
export class EnregistrementEquipeComponent implements OnInit {
  ...
  ngOnInit(): void {
    ...
    if(this.idEquipe > 0){
      ...
      this.cahierQuartService.getOneEnregistrementEquipe(this.idEquipe).subscribe((response) =>{
      ...
      Swal.fire({title: 'Êtes-vous sûr(e) de créer cette équipe ?',icon: 'warning',showCancelButton: true,confirmButtonColor: '#3085d6',cancelButtonColor: '#d33',confirmButtonText: 'Oui, créer',cancelButtonText: 'Annuler'
        ...
        if (result.isConfirmed) {
          ...
          //Sinon on créé l'équipe
            ...
            this.cahierQuartService.nouvelEnregistrementEquipe(nomEquipe).subscribe((response) => {
  ...
  update(){
    this.cahierQuartService.udpateEnregistrementEquipe(this.name,this.idEquipe).subscribe((response) => {      
