...
export class ListEntreeComponent implements OnInit {
  ...
  import(event: Event) {
    ...
    //Bourg en Bresse & Chagny
      ...
      if (this.idUsine === 20) {
        //Bourg en Bresse
        this.lectureCSV(event, ";", true, 13, 3, 5, 9, 4);
      }
      //Chagny
      else {
        this.lectureCSV(event, ";", true, 8, 12, 2, 5);
      }
    }
  }
          ...
          for (let i = debut; i < results.data.length; i++) {
            if (
              ...
              const importCSV = {
                ...
                codeFamilleTransporteur: results.data[i][22].replace(
                  /'/g,
                  "''",
                ),
                ...
                adresseClient:
                  results.data[i][81].replace(/'/g, "''") +
                  results.data[i][82].replace(/'/g, "''"),
    ...
    reader.onload = (event: any) => {
      ...
      this.Papa.parse(csv, {
        ...
        complete: async (results) => {
          ...
          this.moralEntitiesService
            .deleteMesuresEntrantsEntreDeuxDates(dateDeb, dateFin)
            .subscribe((response) => {
    ...
    this.csvArray.forEach((csv) => {
      ...
      this.correspondance.forEach((correspondance) => {
        ...
        if (
          ...
          if (
            correspondance.nomImport == csv.client &&
      ...
      if (
        ...
        this.clientManquants.set(
          dechetManquant + "__" + clientManquant,
          dechetManquant + "__" + clientManquant,
        );
      }
    });
